...
import {
  validateChallengeOwnership,
  validateFromQuestion,
  validateFromOption,
...
export class ChallengesServices {
  ...
  async updateChallengeBase(
    challengeId: string,
    id_company: string | undefined,
    data: ChallengeBaseUpdateDto,
  ) {
  ...
  async updateTechnicalMetadata(
    challengeId: string,
    id_company: string | undefined,
    data: TechnicalMetadataUpdateDto,
  ) {
    return prisma.$transaction(async (tx) => {
      const challenge = await validateChallengeOwnership(
        tx,
        challengeId,
        id_company,
      );
  ...
  async updateNonTechnicalDetails(
    challengeId: string,
    id_company: string | undefined,
    data: NonTechnicalDetailsUpdateDto,
  ) {
    return prisma.$transaction(async (tx) => {
      const challenge = await validateChallengeOwnership(
        tx,
      ...
      if (
        challenge.challenge_type !== ChallengesType.NON_TECHNICAL_CHALLENGES
      ) {
        throw new Error("This challenge is not non-technical");
      }

      const nonTech = await tx.non_technical_challenges.findFirst({
        where: { challenge_id: challengeId },
      });

      if (!nonTech) {
        throw new Error("Non technical data not found");
      }

      await tx.non_technical_challenges.update({
        where: { id: nonTech.id },
        data,
      });
  ...
  async updateNonTechnicalQuestion(
    questionId: string,
    id_company: string | undefined,
    data: NonTechnicalQuestionUpdateDto,
  ) {
    return prisma.$transaction(async (tx) => {
  ...
  async createNonTechnicalQuestion(
  ...
  async updateNonTechnicalOption(
    optionId: string,
    id_company: string | undefined,
    data: NonTechnicalOptionUpdateDto,
  ) {
    return prisma.$transaction(async (tx) => {
  ...
  async createNonTechnicalOption(
    questionId: string,
    id_company: string | undefined,
  ...
  async getChallengeSubmissions(id_challenge: string, id_company: string) {
    ...
    if (id_company && challenge.created_by_company !== id_company) {
      throw new Error(
        "Unauthorized: This challenge does not belong to your company",
      );
    }
